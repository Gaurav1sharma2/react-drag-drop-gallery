...
test.describe('Drag & Drop Gallery E2E Tests', () => {
  ...
  test('TEST 4️⃣: Drag and Drop Reorders Gallery Items', async ({ page }) => {
    ...
    const firstImageTitleBefore = await firstItemBefore.locator('.image-title').textContent();
    ...
    const firstImageTitleAfter = await firstItemAfter.locator('.image-title').textContent();
    
    // The first item should now be different
    expect(firstImageTitleAfter).not.toBe(firstImageTitleBefore);
  });
